namespace GromCore.Laser.Titan.DataStream
{
    ...
    public class BitStream
    {
        ...
        public BitStream Clone()
        {
            ...
            int copyLength = System.Math.Min(length, buffer.Length);
            BitStream clone = new BitStream(copyLength);
            Buffer.BlockCopy(this.buffer, 0, clone.buffer, 0, copyLength);
            return clone;
        }

        public byte ReadBit()
        {
            if (buffer == null || _offset >= buffer.Length)
                return 0;
        ...
        public void WritePositiveVIntMax16777215(int value)
        {
        ...
        public void WriteBit(byte data)
        {
            ...
            if (_offset <= 0 || _offset > buffer.Length)
                return;

            int value = buffer[_offset - 1];
            value &= ~(1 << _bitOffset);
            value |= (data & 1) << _bitOffset;
        ...
        public void EnsureCapacity(int capacity)
        {
            ...
            if (buffer == null)
            {
                buffer = new byte[System.Math.Max(capacity, 16)];
            ...
            int newSize = System.Math.Max(bufferLength * 2, _offset + capacity);
            var tmpBuffer = new byte[newSize];
